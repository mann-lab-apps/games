...
source_dir="$repo_root/prototypes/dopamine-swap/Builds/WebGL/dopamine-swap"
target_dir="$repo_root/web/mannlab-games/public/games/dopamine-swap"

if [[ ! -f "$source_dir/index.html" ]]; then
  echo "WebGL build not found: $source_dir/index.html" >&2
  echo "Run ./scripts/verify-dopamine-swap-webgl.sh first." >&2
  exit 2
fi
...
for asset in data framework.js wasm; do
  gz_asset="$target_dir/Build/dopamine-swap.$asset.gz"
  if [[ -f "$gz_asset" ]]; then
    gzip -dc "$gz_asset" > "$target_dir/Build/dopamine-swap.$asset"
  fi
done

perl -pi -e 's/dopamine-swap\.data\.gz/dopamine-swap.data/g; s/dopamine-swap\.framework\.js\.gz/dopamine-swap.framework.js/g; s/dopamine-swap\.wasm\.gz/dopamine-swap.wasm/g' "$target_dir/index.html"
...
asset_version="$(shasum -a 256 "$target_dir/Build/dopamine-swap.wasm" | awk '{ print substr($1, 1, 12) }')"
...
html = html.replace(
  'var buildUrl = "Build";\n      var loaderUrl = buildUrl + "/dopamine-swap.loader.js";',
  `var buildUrl = "Build";
      var assetVersion = "${assetVersion}";
      var versionSuffix = "?v=" + assetVersion;
      var loaderUrl = buildUrl + "/dopamine-swap.loader.js" + versionSuffix;`
);
html = html
  .replace('dataUrl: buildUrl + "/dopamine-swap.data"', 'dataUrl: buildUrl + "/dopamine-swap.data" + versionSuffix')
  .replace('frameworkUrl: buildUrl + "/dopamine-swap.framework.js"', 'frameworkUrl: buildUrl + "/dopamine-swap.framework.js" + versionSuffix')
  .replace('codeUrl: buildUrl + "/dopamine-swap.wasm"', 'codeUrl: buildUrl + "/dopamine-swap.wasm" + versionSuffix');
html = html.replace(
...
rm -f "$target_dir/Build/dopamine-swap.data.gz" \
  "$target_dir/Build/dopamine-swap.framework.js.gz" \
  "$target_dir/Build/dopamine-swap.wasm.gz"
