...
build_log="/tmp/dopamine-swap-unity-webgl-build.log"
build_output="$project/Builds/WebGL/dopamine-swap"
missing=0
...
"$unity_editor" \
  ...
  -quit \
  -projectPath "$project" \
  -executeMethod MannLab.Games.DopamineSwap.EditorTools.BuildWebGL.Build \
  -logFile "$build_log"
