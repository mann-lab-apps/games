...
import_log="/tmp/drum-duel-unity-import.log"
...
"$unity_editor" \
  ...
  -quit \
  -projectPath "$project" \
  -executeMethod MannLab.Games.DrumDuel.EditorTools.CreateGameScene.Create \
  -logFile "$import_log"

test -f "$project/Assets/_Project/Scenes/Game.unity"

echo "Unity import log: $import_log"
echo "Drum Duel scene verified: $project/Assets/_Project/Scenes/Game.unity"
