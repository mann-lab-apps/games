...
const uploadDir = resolve(repoRoot, "prototypes/2048-crash/Assets/_Project/Art/AppStore/Upload");
...
const shots = [
  { file: "01-start-board.png", moves: [] },
  { file: "02-after-first-slides.png", moves: ["ArrowLeft", "ArrowUp", "ArrowRight"] },
  { file: "03-building-the-crash.png", moves: ["ArrowDown", "ArrowLeft", "ArrowUp", "ArrowRight", "ArrowDown"] },
  { file: "04-late-board.png", moves: ["ArrowLeft", "ArrowUp", "ArrowRight", "ArrowDown", "ArrowLeft", "ArrowUp", "ArrowRight"] },
];
...
function writeFolderMetaIfMissing(dirPath) {
  ...
  writeFilePath(metaPath, `fileFormatVersion: 2
guid: ${unityGuid()}
folderAsset: yes
DefaultImporter:
  externalObjects: {}
...
function writeFilePath(path, contents) {
  ...
  while (Date.now() < deadline) {
    const result = await client.send("Runtime.evaluate", {
      ...
      expression: `(() => {
        ...
        return {
          ready: Boolean(canvas) && loadingDisplay === "none" && !warningText.includes("does not support WebGL"),
...
async function pressKey(client, key) {
  ...
  await client.send("Input.dispatchKeyEvent", { type: "keyDown", key, code: key, windowsVirtualKeyCode: keyCode, nativeVirtualKeyCode: keyCode });
  await delay(80);
  await client.send("Input.dispatchKeyEvent", { type: "keyUp", key, code: key, windowsVirtualKeyCode: keyCode, nativeVirtualKeyCode: keyCode });
  await delay(560);
}
...
async function captureDevice(client, device, appUrl) {
  ...
  await client.send("Runtime.evaluate", {
    expression: `(() => {
      ...
      style.textContent = "*, #unity-canvas { outline: none !important; -webkit-tap-highlight-color: transparent !important; }";
      document.head.appendChild(style);
    })()`,
  });
  await client.send("Runtime.evaluate", { expression: `document.querySelector("#unity-canvas")?.focus()` });

  for (const shot of shots) {
    for (const move of shot.moves) await pressKey(client, move);
    await delay(900);
...
async function main() {
  ...
  if (metaOnly) {
    ...
    console.log(`Generated missing Unity metadata for 2048 Crash App Store screenshots in ${uploadDir}`);
    return;
  }
  ...
  const appUrl = `http://127.0.0.1:${httpPort}/games/2048-crash/`;
  const userDataDir = resolve(repoRoot, `tmp/chrome-2048-crash-capture-${debugPort}`);
  ...
  try {
    await waitForHttp(`http://127.0.0.1:${httpPort}/games/2048-crash/index.html`);
