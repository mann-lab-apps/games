...
project="$repo_root/prototypes/flying-bird"
...
build_log="/tmp/flying-bird-unity-webgl-build.log"
build_output="$project/Builds/WebGL/flying-bird"
missing=0
...
"$unity_editor" \
  ...
  -quit \
  -projectPath "$project" \
  -executeMethod MannLab.Games.FlyingBird.EditorTools.BuildWebGL.Build \
  -logFile "$build_log"
