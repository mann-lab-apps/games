...
const buildDir = resolve(repoRoot, "prototypes/yacht-rush/Builds/WebGL/yacht-rush");
const uploadDir = resolve(repoRoot, "prototypes/yacht-rush/Assets/_Project/Art/AppStore/Upload");
...
async function applyPageCleanups(client) {
  await client.send("Runtime.evaluate", {
    expression: `(() => {
      ...
      style.textContent = "*, #unity-canvas { outline: none !important; -webkit-tap-highlight-color: transparent !important; } html, body { margin: 0 !important; overflow: hidden !important; background: #f8f5ed !important; }";
...
async function main() {
  ...
  if (metaOnly) {
    ...
    console.log(`Generated missing Unity metadata for Yacht Rush screenshots in ${uploadDir}`);
    return;
  }

  if (!existsSync(buildDir)) throw new Error(`Yacht Rush WebGL build not found: ${buildDir}`);
  if (!existsSync(chromePath)) throw new Error(`Google Chrome not found: ${chromePath}`);

  const httpPort = await findFreePort();
  const debugPort = await findFreePort();
  const userDataDir = resolve(repoRoot, `tmp/chrome-yacht-rush-capture-${debugPort}`);
  ...
  try {
    await waitForHttp(`http://127.0.0.1:${httpPort}/index.html`);
    ...
    try {
      for (const device of devices) {
        ...
        for (const shot of shots) {
          const appUrl = `http://127.0.0.1:${httpPort}/index.html?captureShot=${shot.shot}&cacheBust=${Date.now()}`;
  ...
  console.log(`Generated Yacht Rush App Store screenshots in ${uploadDir}`);
}
