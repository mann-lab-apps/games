...
build_log="/tmp/drum-duel-unity-webgl-build.log"
build_output="$project/Builds/WebGL/drum-duel"
missing=0
...
"$unity_editor" \
  ...
  -quit \
  -projectPath "$project" \
  -executeMethod MannLab.Games.DrumDuel.EditorTools.BuildWebGL.Build \
  -logFile "$build_log"
