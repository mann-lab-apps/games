...
source_dir="$repo_root/prototypes/2048-blink/Builds/WebGL/2048-blink"
target_dir="$repo_root/web/mannlab-games/public/games/2048-blink"

if [[ ! -f "$source_dir/index.html" ]]; then
  echo "WebGL build not found: $source_dir/index.html" >&2
  echo "Run ./scripts/verify-2048-blink-webgl.sh first." >&2
  exit 2
fi
...
code_asset="$(find "$target_dir/Build" -maxdepth 1 -type f \( \
  -name "2048-blink.wasm" -o \
  -name "2048-blink.wasm.gz" -o \
  -name "2048-blink.wasm.br" -o \
  -name "2048-blink.wasm.unityweb" \
\) | sort | head -1)"
...
html = html.replace(
  'var buildUrl = "Build";\n      var loaderUrl = buildUrl + "/2048-blink.loader.js";',
  `var buildUrl = "Build";
      var assetVersion = "${assetVersion}";
      var versionSuffix = "?v=" + assetVersion;
      var loaderUrl = buildUrl + "/2048-blink.loader.js" + versionSuffix;`
);
html = html
  .replace(/(dataUrl: buildUrl \+ "\/2048-blink\.data(?:\.(?:gz|br|unityweb))?")/, "$1 + versionSuffix")
  .replace(/(frameworkUrl: buildUrl \+ "\/2048-blink\.framework\.js(?:\.(?:gz|br|unityweb))?")/, "$1 + versionSuffix")
  .replace(/(codeUrl: buildUrl \+ "\/2048-blink\.wasm(?:\.(?:gz|br|unityweb))?")/, "$1 + versionSuffix");
html = html.replace(
...
appSource = appSource.replace(
  /embedHref: "\/games\/2048-blink\/(?:index\.html)?(?:\?v=[a-f0-9]+)?"/,
  `embedHref: "/games/2048-blink/index.html?v=${assetVersion}"`
);
