...
require_text "$controller" "FirebaseTelemetry.SetContext(\"game\", \"2048-blink\")"
...
require_text "$readme" "Firebase/Crashlytics telemetry"

warn_or_fail_missing_config "$ios_plist"

if [[ -f "$ios_plist" ]]; then
  require_text "$ios_plist" "<string>com.mannlab.games.game2048blink</string>"
fi

if [[ "$failures" -ne 0 ]]; then
  echo "2048 Blink Firebase readiness check failed." >&2
  exit 1
fi

if [[ "$warnings" -ne 0 ]]; then
  echo "2048 Blink Firebase code readiness verified; Firebase config files are still needed."
else
  echo "2048 Blink Firebase readiness verified."
fi
