...
build_log="/tmp/2048-blink-unity-webgl-build.log"
build_output="$project/Builds/WebGL/2048-blink"
missing=0
...
if [[ ! -x "$unity_cli" ]]; then
  ...
  if ! python3 -c 'import json,sys; data=json.load(sys.stdin).get("data") or []; sys.exit(0 if len(data) > 0 else 1)' <<< "$license_state"; then
...
"$unity_editor" \
  ...
  -quit \
  -projectPath "$project" \
  -executeMethod MannLab.Games.Game2048Blink.EditorTools.BuildWebGL.Build \
  -logFile "$build_log"
