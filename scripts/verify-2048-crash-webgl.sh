...
build_log="/tmp/2048-crash-unity-webgl-build.log"
build_output="$project/Builds/WebGL/2048-crash"
missing=0
...
"$unity_editor" \
  ...
  -quit \
  -projectPath "$project" \
  -executeMethod MannLab.Games.Game2048Crash.EditorTools.BuildWebGL.Build \
  -logFile "$build_log"
