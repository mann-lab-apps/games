...
const uploadDir = resolve(repoRoot, "prototypes/standing/Assets/_Project/Art/AppStore/Upload");
...
async function waitForHttp(url, timeoutMs = 15000) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      // Keep waiting for the local server or Chrome debug endpoint.
    }
    await delay(250);
  }
...
async function applyPageCleanups(client) {
  await client.send("Runtime.evaluate", {
    expression: `(() => {
      ...
      style.textContent = "*, #unity-canvas { outline: none !important; -webkit-tap-highlight-color: transparent !important; } body { margin: 0 !important; overflow: hidden !important; background: #f6f0e4 !important; }";
...
async function touchStart(client, device) {
  await client.send("Input.dispatchTouchEvent", {
    type: "touchStart",
    touchPoints: [{
      x: device.width * 0.5,
      y: device.height * 0.78,
      radiusX: 12,
      radiusY: 12,
      force: 1,
      id: 1,
...
async function captureDevice(client, device, appUrl) {
  ...
  for (let index = 0; index < shots.length; index++) {
    ...
    await client.send("Page.navigate", { url: `${appUrl}${separator}appstoreShot=${index + 1}` });
    await waitForGame(client);
    await applyPageCleanups(client);
    await delay(9800);
    await capture(client, device, shots[index]);
  }
}
...
async function main() {
  ...
  if (metaOnly) {
    ...
    console.log(`Generated missing Unity metadata for Standing App Store screenshots in ${uploadDir}`);
    return;
  }
  ...
  const appUrl = `http://127.0.0.1:${httpPort}/games/standing/`;
  const userDataDir = resolve(repoRoot, `tmp/chrome-standing-capture-${debugPort}`);
  ...
  try {
    await waitForHttp(`http://127.0.0.1:${httpPort}/games/standing/index.html`);
