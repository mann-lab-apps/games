...
const uploadDir = resolve(repoRoot, "prototypes/gather-and-shot/Assets/_Project/Art/AppStore/Upload");
const chromePath = process.env.CHROME_PATH ?? "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome";
const metaOnly = process.argv.includes("--meta-only");

const devices = [
  { name: "iPhone-6.5", width: 1242, height: 2688, mobile: true, userAgent: "Mozilla/5.0 (iPhone; CPU iPhone OS 18_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/18.0 Mobile/15E148 Safari/604.1" },
...
async function applyPageCleanups(client) {
  await client.send("Runtime.evaluate", {
    expression: `(() => {
      ...
      style.textContent = "*, #unity-canvas { outline: none !important; -webkit-tap-highlight-color: transparent !important; } html, body { margin: 0 !important; overflow: hidden !important; background: #000 !important; }";
...
async function main() {
  ...
  if (metaOnly) {
    ...
    console.log(`Generated missing Unity metadata for Gather & Shot App Store screenshots in ${uploadDir}`);
    return;
  }
  ...
  const appUrl = `http://127.0.0.1:${httpPort}/games/gather-and-shot/`;
  const userDataDir = resolve(repoRoot, `tmp/chrome-gather-and-shot-capture-${debugPort}`);
  ...
  try {
    await waitForHttp(`http://127.0.0.1:${httpPort}/games/gather-and-shot/index.html`);
