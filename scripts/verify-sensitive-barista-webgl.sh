...
webgl_engine="/Applications/Unity/Hub/Editor/6000.3.23f1/PlaybackEngines/WebGLSupport"
build_log="/tmp/sensitive-barista-unity-webgl-build.log"
build_output="$project/Builds/WebGL/sensitive-barista"
missing=0
...
if [[ ! -d "$webgl_engine" ]]; then
  ...
  echo "Install it from Unity Hub > Installs > 6000.3.23f1 > Add modules > Web Build Support." >&2
  missing=1
fi
...
"$unity_editor" \
  ...
  -quit \
  -projectPath "$project" \
  -executeMethod MannLab.Games.SensitiveBarista.EditorTools.BuildWebGL.Build \
  -logFile "$build_log"
