...
if [[ "$mode" != "admob-test" ]]; then
  ...
  if [[ -z "${MANNLAB_ONE_PLUS_ONE_MINUS_ONE_ADMOB_ANDROID_INTERSTITIAL_ID:-}" ]]; then
    ...
    warn_or_fail_invalid_admob_id "Production Android interstitial ad unit env has invalid format." "${MANNLAB_ONE_PLUS_ONE_MINUS_ONE_ADMOB_ANDROID_INTERSTITIAL_ID:-}" '^ca-app-pub-[0-9]{16}/[0-9]{10}$'
  fi
  ...
  if [[ -n "${MANNLAB_ONE_PLUS_ONE_MINUS_ONE_ANDROID_KEYSTORE_PATH:-}" &&
    ...
    if [[ "${MANNLAB_ONE_PLUS_ONE_MINUS_ONE_ANDROID_KEYSTORE_PATH:-}" != /* ]]; then
      ...
      warn_or_fail "MANNLAB_ONE_PLUS_ONE_MINUS_ONE_ANDROID_KEYSTORE_PATH does not exist." "${REQUIRE_ANDROID_SIGNING_ENV:-0}"
    fi
  fi
  ...
  if [[ -z "${MANNLAB_ONE_PLUS_ONE_MINUS_ONE_ANDROID_MARKETING_VERSION:-}" ]]; then
    ...
    warn_or_fail_invalid_env_format \
      "MANNLAB_ONE_PLUS_ONE_MINUS_ONE_ANDROID_MARKETING_VERSION should look like 1.0 or 1.0.0." \
