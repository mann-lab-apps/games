...
if [[ -f "$target_dir/index.html" ]]; then
  perl -0pi -e 's/\.data\.gz/.data/g; s/\.framework\.js\.gz/.framework.js/g; s/\.wasm\.gz/.wasm/g; s#(dataUrl: buildUrl \+ "/walking\.data")#$1 + "?v=imgui-20260830"#g; s#(frameworkUrl: buildUrl \+ "/walking\.framework\.js")#$1 + "?v=imgui-20260830"#g; s#(codeUrl: buildUrl \+ "/walking\.wasm")#$1 + "?v=imgui-20260830"#g; s/canvas\.style\.width = "960px";/canvas.style.width = "100vw";/g; s/canvas\.style\.height = "600px";/canvas.style.height = "100vh";/g' "$target_dir/index.html"
fi
