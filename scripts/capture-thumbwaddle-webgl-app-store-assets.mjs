...
const uploadDir = resolve(repoRoot, "prototypes/walking/Assets/_Project/Art/AppStore/Upload");
...
const shots = [
  "01-ready-go-far.png",
  "02-first-left-right-step.png",
  "03-iceberg-path.png",
  "04-distance-rhythm-run.png",
  "05-result-best-waddle.png",
];
...
async function applyPageCleanups(client) {
  await client.send("Runtime.evaluate", {
    expression: `(() => {
      localStorage.clear();
      const style = document.createElement("style");
      style.textContent = "*, #unity-canvas { outline: none !important; -webkit-tap-highlight-color: transparent !important; } html, body { margin: 0 !important; overflow: hidden !important; background: #e2f6fa !important; } #unity-footer { display: none !important; } #unity-container, #unity-canvas { width: 100vw !important; height: 100vh !important; }";
...
async function touchStep(client, device, side, yTopRatio = 0.34, xBias = 0) {
  const x = Math.round(device.width * (side === "left" ? 0.25 + xBias : 0.75 + xBias));
  const startY = Math.round(device.height * yTopRatio);
  const endY = Math.round(device.height * 0.84);
  await client.send("Input.dispatchTouchEvent", {
    type: "touchStart",
    touchPoints: [{ x, y: startY, radiusX: 12, radiusY: 12, force: 1, id: side === "left" ? 1 : 2 }],
  });
  await delay(95);
  const steps = 8;
...
async function waddle(client, device, count, options = {}) {
  for (let i = 0; i < count; i++) {
    const side = i % 2 === 0 ? "left" : "right";
    const lean = options.weave ? Math.sin(i * 0.88) * 0.055 : 0;
    const yTopRatio = options.long ? 0.28 : 0.34;
    await touchStep(client, device, side, yTopRatio, lean);
  }
}
...
async function captureDevice(client, device, appUrl) {
  ...
  await delay(900);
  await capture(client, device, shots[0]);

  await waddle(client, device, 2);
  await delay(450);
  await capture(client, device, shots[1]);

  await waddle(client, device, 10, { weave: true, long: true });
  await delay(900);
  await capture(client, device, shots[2]);

  await waddle(client, device, 10, { weave: true, long: true });
  await delay(800);
  await capture(client, device, shots[3]);

  await waddle(client, device, 12, { weave: true, long: true });
  await delay(12500);
  await capture(client, device, shots[4]);
}
...
async function main() {
  ...
  if (metaOnly) {
    for (const device of devices) {
      ...
      for (const shot of shots) {
        const pngPath = resolve(uploadDir, device.name, shot);
        if (existsSync(pngPath)) writeTextureMetaIfMissing(pngPath);
      }
    }
    console.log(`Generated missing Unity metadata for Thumbwaddle App Store screenshots in ${uploadDir}`);
    return;
  }
  ...
  const appUrl = `http://127.0.0.1:${httpPort}/games/thumbwaddle/`;
  const userDataDir = resolve(repoRoot, `tmp/chrome-thumbwaddle-capture-${debugPort}`);
  ...
  try {
    await waitForHttp(`http://127.0.0.1:${httpPort}/games/thumbwaddle/index.html`);
