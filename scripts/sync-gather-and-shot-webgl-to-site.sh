...
source_dir="$repo_root/prototypes/gather-and-shot/Builds/WebGL/gather-and-shot"
target_dir="$repo_root/web/mannlab-games/public/games/gather-and-shot"

if [[ ! -f "$source_dir/index.html" ]]; then
  echo "WebGL build not found: $source_dir/index.html" >&2
  echo "Run ./scripts/verify-gather-and-shot-webgl.sh first." >&2
  exit 2
fi
...
for asset in data framework.js wasm; do
  gz_asset="$target_dir/Build/gather-and-shot.$asset.gz"
  if [[ -f "$gz_asset" ]]; then
    gzip -dc "$gz_asset" > "$target_dir/Build/gather-and-shot.$asset"
  fi
done
...
asset_version="$(
  shasum -a 256 \
    ...
    "$target_dir/Build/gather-and-shot.framework.js" \
    "$target_dir/Build/gather-and-shot.wasm" \
    "$target_dir/Build/gather-and-shot.loader.js" |
    shasum -a 256 |
...
html = html.replace(
  'var buildUrl = "Build";\n      var loaderUrl = buildUrl + "/gather-and-shot.loader.js";',
  `var buildUrl = "Build";
      var assetVersion = "${assetVersion}";
      var versionSuffix = "?v=" + assetVersion;
      var loaderUrl = buildUrl + "/gather-and-shot.loader.js" + versionSuffix;`
);
html = html
  .replace('dataUrl: buildUrl + "/gather-and-shot.data"', 'dataUrl: buildUrl + "/gather-and-shot.data" + versionSuffix')
  .replace('frameworkUrl: buildUrl + "/gather-and-shot.framework.js"', 'frameworkUrl: buildUrl + "/gather-and-shot.framework.js" + versionSuffix')
  .replace('codeUrl: buildUrl + "/gather-and-shot.wasm"', 'codeUrl: buildUrl + "/gather-and-shot.wasm" + versionSuffix');
html = html.replace(
...
hub = hub.replace(
  /embedHref: "\/games\/gather-and-shot\/(?:index\.html)?(?:\?v=[0-9a-f]+)?"/,
  `embedHref: "/games/gather-and-shot/?v=${assetVersion}"`
);
fs.writeFileSync(hubPath, hub);
NODE

rm -f "$target_dir/Build/gather-and-shot.data.gz" \
  "$target_dir/Build/gather-and-shot.framework.js.gz" \
  "$target_dir/Build/gather-and-shot.wasm.gz"
